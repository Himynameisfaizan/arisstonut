...
if (isset($_POST['action']) && $_POST['action'] == 'add_to_cart') {
    ...
    if ($pid > 0) {
        ...
        if (array_key_exists($cart_key, $_SESSION['cart'])) {
            ...
            $_SESSION['cart'][$cart_key] = ['id' => $pid, 'variation_id' => $vid, 'quantity' => $qty];
        }
...
if (isset($_POST['action']) && $_POST['action'] == 'update_quantity') {
    ...
    if (isset($_SESSION['cart'][$cart_key])) {
        ...
        } elseif ($type == 'dec' && $_SESSION['cart'][$cart_key]['quantity'] > 1) {
            $_SESSION['cart'][$cart_key]['quantity']--;
        }
        ...
        if ($vid > 0) {
            ...
            if ($curr_qty >= 6 && $v_data['price_6_plus'] > 0)
                $item_price = $v_data['price_6_plus'];
            elseif ($curr_qty >= 5 && $v_data['price_5_plus'] > 0)
                $item_price = $v_data['price_5_plus'];
            elseif ($curr_qty >= 4 && $v_data['price_4_plus'] > 0)
                $item_price = $v_data['price_4_plus'];
        } else {
            $query = $conn->query("SELECT selling_price FROM products WHERE id = '$pid'");
            $item_price = $query->fetch_assoc()['selling_price'];
        }
        ...
        foreach ($_SESSION['cart'] as $k => $item) {
            ...
            if ($i_vid > 0) {
                ...
                if ($i_qty >= 6 && $vd['price_6_plus'] > 0)
                    $i_price = $vd['price_6_plus'];
                elseif ($i_qty >= 5 && $vd['price_5_plus'] > 0)
                    $i_price = $vd['price_5_plus'];
                elseif ($i_qty >= 4 && $vd['price_4_plus'] > 0)
                    $i_price = $vd['price_4_plus'];
            } else {
                $q = $conn->query("SELECT selling_price FROM products WHERE id = '$i_pid'");
                $i_price = $q->fetch_assoc()['selling_price'];
            }
            $grand_total += ($i_price * $i_qty);
            $total_items += $i_qty;
        }

        echo json_encode(['status' => 'success', 'quantity' => $curr_qty, 'subtotal' => number_format($new_subtotal, 2), 'grand_total' => number_format($grand_total, 2), 'cart_count' => $total_items]);
    } else {
...
if (isset($_POST['action']) && $_POST['action'] == 'remove_item') {
    ...
    if (isset($_SESSION['cart'][$cart_key])) {
        ...
        echo json_encode(['status' => 'success', 'cart_empty' => empty($_SESSION['cart']), 'grand_total' => number_format($grand_total, 2), 'cart_count' => $total_items]);
    } else {
        echo json_encode(['status' => 'error']);
    }
    exit();
...
// ==========================================
// NEW: BUY NOW SYSTEM EXECUTION MODULE
// ==========================================
