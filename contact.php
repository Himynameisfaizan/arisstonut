...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_contact'])) {
    // Input parameters sanitization filter mapping matrix
    $name    = htmlspecialchars(trim($_POST['full_name']));
    $email   = filter_var(trim($_POST['email']), FILTER_SANITIZE_EMAIL);
    $subject = htmlspecialchars(trim($_POST['subject']));
    $message = htmlspecialchars(trim($_POST['message']));

    // Server end validation checks logic pipeline
    if (empty($name) || empty($email) || empty($subject) || empty($message)) {
        ...
        // Columns sequence format mapping: name, email, subject, message
        ...
        if ($db_execute_success) {
            ...
            // 2. CORE TARGET EMAIL MODULE SEGMENT
            $to = "aristonut@gmail.com"; 
            
            // Structured HTML Layout Email Body Build Matrix
            $email_subject = "AristoNut Contact Form: " . $subject;
            
            $email_body = "
            <html>
            <head>
                <title>New Message From AristoNut</title>
            </head>
            <body style='font-family: Arial, sans-serif; background-color: #fffaf5; padding: 20px; color: #3e2723;'>
            ...
            // Secure boundary headers configuration blocks mapping layout
            $headers  = "MIME-Version: 1.0" . "\r\n";
            $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
            $headers .= "From: AristoNut Webportal <no-reply@blue-okapi-934452.hostingersite.com>" . "\r\n";
            $headers .= "Reply-To: " . $email . "\r\n";

            // Execute backend delivery execution process trace routing
            mail($to, $email_subject, $email_body, $headers);

            // Set dynamic message view for client UI screen persistence state
            $msg_status = "🎉 Your inquiry has been submitted and recorded inside our central sheets database! Our support loop will trace back to you shortly.";
            $msg_class = "alert-success";
            
            // Clean local memory variables inputs logic to avoid form persistence behavior
